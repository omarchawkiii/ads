        ...
        success: function (response) {
            try {
                ...
                if (running_ingest.length === 0) {
                    ...
                    $('#empty-running-task').removeClass("hide_div");
                } else {
                    $('#empty-running-task').addClass("hide_div");
                ...
                if (pending_ingest.length === 0) {
                    ...
                    $('#empty-pending-task').removeClass("hide_div");
                }else{
                    $('#empty-pending-task').addClass("hide_div");
                ...
                if (dcp.length === 0) {
                    ...
                    for (var i = 0; i < dcp.length; i++) {
                        box_logs += '<tr class="item_to_select logs-item" ' +
                            ...
                            '<td class="cpl-item">' +
                            '<a class="text-body align-middle fw-medium text-decoration-none">' +
                            '<span class="btn btn-primary custom-search mdi mdi-magnify logs_details" data-id="' + dcp[i].cpl_id + '"></span>' +
                            '</a></td>' +
                            '</tr>';
                    }
                    ...
                    if ($.fn.DataTable.isDataTable('#table_ingest_logs')) {
                        $('#table_ingest_logs').DataTable().destroy();
                    }

                    $('#table_ingest_logs').DataTable({
                        pageLength: 50,
                        language: {
                            search: "_INPUT_",
    ...
    function displayIngestLogs() {
        ...
        $('#table_ingest_logs').DataTable({
            ...
            ajax: {
                ...
                data: { action_control: "get_logs" } // plus besoin de dataSrc ici
            },
            columns: [
                {
                    data: 'status',
                    render: function (data) {
                        return getStatusDownload(data);
                    }
                },
                { data: 'cpl_description' },
                { data: 'source' },
                { data: 'type' },
                {
                    data: 'date_start_ingesting',
                    render: function (data, type, row) {
                        return data ?? row.date_create_ingest;
                    }
                },
                {
                    data: 'date_finished_ingest',
                    render: function (data, type, row) {
                        return data ?? row.date_create_ingest;
                    }
                },
                {
                    data: 'percentage',
                    render: function (data, type, row) {
                        let percent = row.status === "Complete" && (!row.Total_size || row.Total_size === 0) ? 100 : data;
                        return percent + ' %';
                    }
                },
                {
                    ...
                    render: function (data) {
                        return `
                            <a class="text-body align-middle fw-medium text-decoration-none">
                                <span class="btn btn-primary custom-search mdi mdi-magnify logs_details" data-id="${data}"></span>
                            </a>`;
                    }
                }
            ],
...
function logs_details(id)
{
    console.log('detail') ;
    $.ajax({
        url: '/ingester/ingest_logs_detail',
        method: "POST",
        data: {id: id},
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        success: function (response) {
            ...
            try {
                ...
                $('#details-ingest-modal2 #detail_log_title').html(response.ingests.cpl_description)
                $('#details-ingest-modal2 #detail_log_uuid').html(response.ingests.cpl_id)
                $('#details-ingest-modal2 #detail_log_pkl').html(response.ingests.pkl_description)
                $('#details-ingest-modal2 #detail_log_pkl').html(response.ingests.pkl_description)
                $('#details-ingest-modal2 #detail_log_asset').html(response.ingests.asset_description)
                $('#details-ingest-modal2 #detail_log_Source').html(response.ingests.name_source)
                $('#details-ingest-modal2 #detail_log_status').html(response.ingests.status)
                $('#details-ingest-modal2 #detail_log_date_create').html(response.ingests.date_create_ingest)
                $('#details-ingest-modal2 #detail_log_date_start').html(response.ingests.date_start_ingesting)
                $('#details-ingest-modal2 #detail_log_date_end').html(response.ingests.date_end_ingest)
                $('#details-ingest-modal2 #detail_log_path').html(response.ingests.tms_dir)


                var ingests_dcp_logs =""
                $.each(response.ingests_dcp_logs, function( index, value ) {
                    var download ="";
                    var OriginalFileName =""
                    var Type =""
                    if(value.Size == value.progress)
                    {
                        download ="Downloaded"
                    }
                    else
                    ...
                    {
                        OriginalFileName =' FileName : ' + value.OriginalFileName
                    }
                    else
                    {
                        OriginalFileName=""
                    ...
                    ingests_dcp_logs = ingests_dcp_logs
                    +'<p class="">'+ OriginalFileName +  Type+ '  </p>'
                    +' <p> Created : '+ value.date_log+' | Download :'+ download +' | '+ value.details+' </p><hr>'
                });

                $.each(response.ingest_dcp_larges, function( index, value ) {
                    var download ="";
                    var hash="";
                    console.log(value.Size)
                    console.log(value.progress)
                    console.log(value.hash_verified)
                    if(value.Size == value.progress)
                    {
                        download ="Downloaded"
                    }
                    ...
                    if(value.hash_verified == "1")
                    {
                        hash ="<span> verified  </span>"
                    }
                    else
                    {
                        hash = '<span style="color : red"> Failed </span>'
                    }


                    ingests_dcp_logs = ingests_dcp_logs
                    +' <p class=""> FileName : '+ value.OriginalFileName+' |  '+ value.Type+'  </p>'
                    +' <p> Created : '+ value.date_create_ingest+' | Download :'+ download +' | Hash Verification : '+ hash+' </p><hr>'
                });

                $("#details-ingest-modal2 #detail_log_ingest_logs").html(ingests_dcp_logs);

                $("#details-ingest-modal2").modal('show');
            } catch (e) {
                console.log(e);
            }
        },
